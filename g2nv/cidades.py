...
class Cidades(SearchDomain):
    ...
    def cost(self, city, action):
        ...
        for (orig, dest, dist) in self.connections:
            if(orig == C1 and dest == C2) or (orig == C2 and dest == C1):
                return dist
    def heuristic(self, city, goal_city):
        x_orig, y_orig = self.coordinates[city]
        x_dest, y_dest = self.coordinates[goal_city]

        x_d = abs(x_orig - x_dest)
        y_d = abs(y_orig - x_dest)
...
tL = [SearchTree(p,'breadth'), SearchTree(p,'depth'), SearchTree(p,'uniform'), SearchTree(p,'greedy'), SearchTree(p, 'astar')]
for t in tL:
    print("Resultado: " + str(t.search(8)))
    print("Comprimento: " + str(t.length))
    print("Terminais: " + str(t.terminal) + " Não terminais: " + str(t.non_terminal))
    print("Ramificação: " + str(t.ramification))
    print("Custo total da Solução: " + str(t.cost))
    print("No/s de maior custo: " + str(t.highest_cost_nodes))
    print("Profundidade média: " + str(t.average_depth))
